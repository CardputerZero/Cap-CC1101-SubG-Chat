...
{
    static const bool enabled = envEnabled("CC1101_CHAT_KEYBOARD_DEBUG", false);
    return enabled;
}
...
{
    if (const char* configured = std::getenv("CC1101_CHAT_KEYMAP_PATH")) {
        return configured;
    }
...
CC1101ChatKeypad::CC1101ChatKeypad()
#if !LV_USE_SDL && defined(__linux__)
    : _cardputer_keymap(cardputerKeymapPath())
#endif
{
#if !LV_USE_SDL && defined(__linux__)
    spdlog::info("CC1101ChatKeypad: {} Cardputer keymap ({} entries)",
...
bool CC1101ChatKeypad::openDefault()
{
#if !LV_USE_SDL && defined(__linux__)
    if (const char* device_path = std::getenv("CC1101_CHAT_KEYBOARD_DEVICE")) {
    ...
    if (!opened) {
        spdlog::warn("CC1101ChatKeypad: no keyboard input device opened");
    }
    return opened;
#else
    return false;
...
bool CC1101ChatKeypad::openDevice(const std::string& path, bool require_app_keys)
{
#if !LV_USE_SDL && defined(__linux__)
    if (!ensureIndev()) {
        return false;
    ...
    const bool grab_input = envEnabled("CC1101_CHAT_KEYBOARD_GRAB", false);
    if (grab_input && ::ioctl(fd, EVIOCGRAB, 1) < 0) {
        spdlog::warn("CC1101ChatKeypad: failed to grab {}: {}", path, std::strerror(errno));
        ::close(fd);
        return false;
    }

    _event_fds.push_back(fd);
    spdlog::info("CC1101ChatKeypad: opened {}{}", path, grab_input ? " with grab" : "");
    return true;
#else
    (void)path;
...
void CC1101ChatKeypad::poll()
{
    ...
    for (int fd : _event_fds) {
        while (true) {
            ...
            if (bytes_read < 0) {
                spdlog::warn("CC1101ChatKeypad: failed to read input event: {}", std::strerror(errno));
            }
            break;
        }
    }
...
lv_indev_t* CC1101ChatKeypad::indev() const
{
    return _indev;
}

void CC1101ChatKeypad::setKeyCallback(KeyCallback callback)
{
    _key_callback = std::move(callback);
}

void CC1101ChatKeypad::readCb(lv_indev_t* indev, lv_indev_data_t* data)
{
    auto* keypad = static_cast<CC1101ChatKeypad*>(lv_indev_get_user_data(indev));
    if (!keypad || !data) {
        return;
    }
...
bool CC1101ChatKeypad::ensureIndev()
{
    if (_indev) {
        return true;
    }
    ...
    if (!_indev) {
        spdlog::error("CC1101ChatKeypad: failed to create LVGL keypad indev");
        return false;
    }
...
void CC1101ChatKeypad::pushKeyEvent(uint16_t code, int32_t value)
{
    if (value != 0 && value != 1) {
#if !LV_USE_SDL && defined(__linux__)
        if (inputDebugEnabled()) {
            spdlog::info("CC1101ChatKeypad: ignored repeat key code={}, value={}", code, value);
        }
#endif
        return;
    }
...
#if !LV_USE_SDL && defined(__linux__)
        if (inputDebugEnabled()) {
            spdlog::info("CC1101ChatKeypad: ignored key code={}, value={}", code, value);
        }
#endif
        return;
    }
...
#if !LV_USE_SDL && defined(__linux__)
    if (inputDebugEnabled()) {
        spdlog::info("CC1101ChatKeypad: key code={}, translated={}, utf8={}, pressed={}", code, key, debugKeyUtf8(key),
                     pressed);
    }
#endif
    bool consumed = false;
...
uint32_t CC1101ChatKeypad::translateKey(uint16_t code) const
{
#if !LV_USE_SDL && defined(__linux__)
...
bool CC1101ChatKeypad::shiftPressed() const
{
    return _left_shift_pressed || _right_shift_pressed;
}

const char* CC1101ChatKeypad::keyUtf8(uint32_t key) const
{
