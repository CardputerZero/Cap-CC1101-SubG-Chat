...
HOME="${HOME:-/tmp/cc1101-chat-home}"
...
SYSROOT_VALIDATION_STAMP="${SYSROOT}/.cc1101-chat-bsp-sha256"
OVERLAY_DIR="${SYSROOT_OVERLAY_DIR:-/opt/cc1101-chat/sysroot-overlay}"
...
if [[ ! -d "${ROOT_DIR}" || ! -f "${ROOT_DIR}/CMakeLists.txt" ]]; then
    echo "Cap-CC1101-SubG-Chat source is not mounted at ${ROOT_DIR}." >&2
    exit 1
fi
...
if [[ ! -e "${SYSROOT}/usr/lib/aarch64-linux-gnu/libgpiod.so.3.1.1" || \
    ...
    if [[ -n "${BSP_SHA256}" ]]; then
        printf "%s\n" "${BSP_SHA256}" >"${EXTRACT_DIR}/.cc1101-chat-bsp-sha256"
    fi
...
CC1101_CHAT_FORCE_CROSS=1 \
CC1101_CHAT_SYSROOT="${SYSROOT}" \
BUILD_DIR="${BUILD_DIR}" \
STAGE_DIR="${STAGE_DIR}" \
